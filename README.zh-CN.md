...
3. 双击运行 **DSH Desktop**。发布版为 ad-hoc 签名但**未公证**——macOS 26 上首次启动会被 Gatekeeper 拦截（旧版「右键 → 打开」绕过已失效）。先尝试打开一次（会被拦截），再到**系统设置 → 隐私与安全性 → 仍要打开**中允许并确认。**每个新版本都需要重复此操作一次。**
...
发布版当前为 **ad-hoc 签名且未公证**——Apple 公证服务对 arm64 提交处理异常，公证已暂时停用。macOS 26 上，下载的构建首次启动会被 Gatekeeper 拦截（旧版「右键 → 打开」绕过已失效）：先尝试打开一次，再到**系统设置 → 隐私与安全性 → 仍要打开**中允许并确认，每个新版本都要重复。Apple arm64 公证管线恢复后将重新启用 Developer ID 签名 + 公证（CI 配置已保留待恢复）。
